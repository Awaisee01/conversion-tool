...
const UnitConverter: React.FC = () => {
  ...
  const convertUnits = (value: string, fromUnit: string, toUnit: string, category: string): string => {
  ...
  const handleFromValueChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    const value: string = e.target.value;
    setFromValue(value);
    
    if (value && !isNaN(Number(value))) {
      const converted: string = convertUnits(value, fromUnit, toUnit, activeTab);
  ...
  const handleToValueChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    const value: string = e.target.value;
    setToValue(value);
    
    if (value && !isNaN(Number(value))) {
      const converted: string = convertUnits(value, toUnit, fromUnit, activeTab);
  ...
  const handleFromUnitChange = (e: React.ChangeEvent<HTMLSelectElement>): void => {
    ...
    if (fromValue) {
      const converted: string = convertUnits(fromValue, e.target.value, toUnit, activeTab);
      setToValue(converted);
    }
  };
  ...
  const handleToUnitChange = (e: React.ChangeEvent<HTMLSelectElement>): void => {
    ...
    if (fromValue) {
      const converted: string = convertUnits(fromValue, fromUnit, e.target.value, activeTab);
      setToValue(converted);
    }
  };

  const currentUnits: string[] = Object.keys(conversionData[activeTab]?.units || {});

  return (
    <div className="max-w-[700px] mx-auto bg-white border border-gray-300 shadow-lg rounded-lg overflow-hidden">
      {/* Header */}
      ...
      <div className="flex  border-b border-gray-300 bg-gray-50">
        {tabs.map((tab: string) => (
          <button
            ...
            className={`sm:px-5 sm:py-4 px-4 py-3 text-[10px] sm:text-sm font-medium transition-colors ${
              activeTab === tab
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full p-3 border border-gray-300 rounded bg-white text-black mb-3 focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent"
            />
            <select
              value={fromUnit}
              onChange={handleFromUnitChange}
              className="w-full p-3 border border-gray-300 rounded bg-white text-black focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent"
            >
        ...
        {fromValue && toValue && (
          <div className="mt-6 p-4 bg-white border border-gray-300 rounded">
            <p className="text-black text-sm">
              ...
              <span className="font-semibold text-black"> {toValue} {toUnit}</span>
            </p>
          </div>
        )}
      </div>
